...
exports.unsetServer = function(Crafty, socket) {
	// set to receive from server
...
function RoomManager(createGame, destroyGame, playerJoin, playerLeave) {
...
RoomManager.prototype.join = function(socket) {
	var npm_crafty = this.npm_crafty,
		self = this;

	socket.on("Player Joined", function (slot, callback) {
		console.log("PLAYER JOINED @", slot);
		self.playerJoin(self.instance);
		callback();
	});
	socket.on("Player Left", function (slot, callback) {
		console.log("PLAYER LEFT @", slot);
		self.playerLeave(self.instance);
		callback();
	});

	socket.emit('Join', function(slot) {
		self.instance = npm_crafty.createClient(slot);
		npm_crafty.setServer(self.instance, socket);

		console.log("JOINED @", slot);
		self.createGame(self.instance);
	});
};

RoomManager.prototype.leave = function(socket) {
	var npm_crafty = this.npm_crafty,
		self = this;

	socket.emit('Leave', function(slot) {
		npm_crafty.unsetServer(self.instance, socket);

		console.log("LEFT @", slot);
		self.destroyGame(self.instance);
	});
