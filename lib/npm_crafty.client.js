...
function RoomManager(reinitGame, startGame, stopGame) {
...
RoomManager.prototype.join = function(socket) {
	...
	socket.on('GameSlot', function (slot) {
		console.log(slot);
		self.instance = npm_crafty.createClient(slot);
		self.reinitGame(self.instance);
		npm_crafty.setServer(self.instance, socket);
	});

	socket.on('GameStart', function() {
		console.log("start");
		self.startGame(self.instance);
	});

	socket.on('GameStop', function() {
		console.log("stop");
		self.stopGame(self.instance);
	});
};
