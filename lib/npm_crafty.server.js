...
RoomManager.prototype.notify = function(room, slot, message, notifyCallback) {
	var self = this;

	var socketIds = Object.keys(room.takenSpots);
	var ackedIds = 0;
	for (var i = 0; i < socketIds.length; ++i) {
		self.namespace.sockets[socketIds[i]].emit(message, slot, function(data) {
			ackedIds++;
			if (ackedIds >= socketIds.length) {
				console.log(message, "@", slot);
				notifyCallback(room.instance, slot, room.openSlots);
			}
		});
	}
};
...
RoomManager.prototype.listen = function(socket) {
	...
	var join = function (callback) {
		var slot = self.allocateSlot(socket),
			room = self.sockets[socket.id];
		console.log("JOINED @", slot);
		callback(slot);

		self.notify(room, slot, "Player Joined", self.playerJoin);
	};
	...
	var leave = function (callback) {
		...
		self.notify(room, slot, "Player Left", self.playerLeave);
	};
