...
# Overview
----------
The unmodified crafty version runs on the client. A heavily modified version runs on the server 
(drawing stuff and similar is omitted from the server version).   
...
hosts will affect how your game sessions are setup. Each game session takes place inside a game room. A game room consists
of one crafty server instance that communicates with multiple client instances.   
